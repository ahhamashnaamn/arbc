...
local function export_chat_link_callback(cb_extra, success, result)
  ...
  save_data(_config.moderation.data, data)
  return send_large_msg(receiver,'Newest generated invite link is:\n'..result)
end
 --DEVELOPER BY @sajadaltaie--
...
local function show_group_settings(msg, data)
  ...
  local text = "Group settings:\nLock group name : "..settings.lock_name.."\nLock group photo : "..settings.lock_photo.."\nLock group member : "..settings.lock_member.."\nLock group spam : "..settings.lock_spam
  return text
end
...
function run(msg, matches)
  ...
  if data[tostring(msg.to.id)] then
    ...
    if matches[1] == 'l' and matches[2] == 'n' then
	  ...
	  -- group link {revoke}
    if matches[1] == 'r' and matches[2] == 'ln' then
      if matches[3] == '' then
        if data[tostring(msg.to.id)]['link'] then
          local link = data[tostring(msg.to.id)]['link']
          return link
      ...
      if matches[3] == 'k' and is_momod(msg) then
        local chat = 'chat#id'..msg.to.id
        msgr = export_chat_link('chat#id'..msg.to.id, export_chat_link_callback, {receiver=receiver, data=data, chat_id=msg.to.id, group_name=msg.to.print_name})
      end
	  end
	  ...
	  -- group link {close}
    if matches[1] == 'c' and matches[2] == 'ln' then
      if matches[3] == '' then
        if data[tostring(msg.to.id)]['link'] then
          local link = data[tostring(msg.to.id)]['link']
          return link
      ...
      if matches[3] == 'k' and is_momod(msg) then
        local chat = 'chat#id'..msg.to.id
        msgr = export_chat_link('chat#id'..msg.to.id, export_chat_link_callback, {receiver=receiver, data=data, group_name=msg.to.print_name})
        return 'تم قفل الرابط..'
      end
	  end
    ...
    if matches[1] == 'g' and matches[2] == 'l' then
    ...
    if matches[1] == 'g' and matches[2] == 'u' then
      if matches[3] == 'name' then
